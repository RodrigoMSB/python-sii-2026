#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────
#  Preparador del Lab 03 (macOS / Linux / Windows con Git Bash)
...
echo "${CIAN}${NEGRITA}Preparando el Lab 03 — Los números del puerto${RESET}"
...
echo "${CIAN}[INFO]${RESET} numpy y pandas pesan unos MB; si tienes proxy/antivirus, ten paciencia."
uv sync

"""La gran limpieza — SOLUCIÓN OFICIAL (Lab 05).

Pipeline que toma el "Censo de Patentes" (fusionado a la rápida por el
practicante, con toda su mugre) y lo deja limpio: homogeneiza el texto,
deduplica, filtra códigos malformados, imputa deudas faltantes y detecta
outliers por DOS métodos (IQR y z-score). La regla de oro: **limpiar no es
borrar, es decidir con criterio y dejar rastro** — cada fila descartada queda
contabilizada y justificada en el reporte.

El clímax: IQR acusa a dos patentes de outliers, z-score solo a una. El analista
descubre que una (Buceo Fondo Claro, viejo conocido desde el Lab 01) es un
negocio real, y la otra ($9.999.999) es un error de digitación. Los métodos
proponen; el analista dispone.
...
# Formato válido de código de patente (regex): PS-####-Y, Y ∈ {C, G, T}.
...
def homogeneizar(df):
    ...
    df = df.copy()  # C13: nunca mutamos el DataFrame recibido
...
def imputar_deuda(df):
    ...
    marca para verificación en terreno". No es la verdad; es una decisión
...
def outliers_iqr(df):
    ...
    return df[(df["deuda"] < bajo) | (df["deuda"] > alto)].copy()


def outliers_z(df, umbral=3.0):
    """Filas cuya deuda tiene |z-score| mayor al umbral (std MUESTRAL, ddof=1)."""
    media = df["deuda"].mean()
    desv = df["deuda"].std()  # C15: pandas usa ddof=1 (muestral) por defecto
...
def limpiar_censo(ruta, umbral_z=3.0):
    ...
    # Detección de outliers por AMBOS métodos sobre el censo ya imputado.
    iqr_df = outliers_iqr(df)
    z_df = outliers_z(df, umbral_z)
    cod_iqr = set(iqr_df["codigo"])
    cod_z = set(z_df["codigo"])

    # Regla de decisión: se APARTA solo lo señalado por AMBOS (consenso = typo).
    # Lo marcado únicamente por IQR se CONSERVA, con nota en el reporte.
    apartados = sorted(cod_iqr & cod_z)

    # Detalle con veredicto por outlier (para el informe).
    detalle = []

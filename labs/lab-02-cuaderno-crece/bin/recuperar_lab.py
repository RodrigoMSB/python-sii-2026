"""Recuperador del Lab 02 — modo instructor / rezagado.

Reconstruye el CÓDIGO y las SALIDAS desde la solución oficial:

    uv run python bin/recuperar_lab.py

Único script del lab que escribe. Hace tres cosas:
  1. Copia soluciones/consolidar.py → consolidar.py (en la raíz).
  2. Ejecuta el programa para regenerar salidas/informe_consolidacion.txt.
  3. Copia plantillas/RESPUESTAS.md → RESPUESTAS.md SOLO si no existe.

Nunca responde el interrogatorio por ti. Recuperar el código es gratis;
recuperar la comprensión, jamás.
"""
...
def main() -> int:
    lc.titulo("Recuperador del Lab 02")
    cont = lc.Contador()

    origen = RAIZ / "soluciones" / "consolidar.py"
    destino = RAIZ / "consolidar.py"
    if origen.is_file():
        shutil.copyfile(origen, destino)
        lc.ok("consolidar.py restaurado desde soluciones/.", cont)
    else:
        lc.error("No encuentro soluciones/consolidar.py.", "¿Lab completo? Vuelve a clonar.", cont)
    ...
    if destino.is_file():
        ...
        if resultado.returncode == 0 and (RAIZ / "salidas" / "informe_consolidacion.txt").is_file():
            ...
            pista = (resultado.stderr.strip().splitlines()[-1]
                     if resultado.stderr.strip() else "Revisa datos/archivador.py.")
            lc.error("No pude regenerar el informe ejecutando el programa.", pista, cont)
    else:
        lc.error("No hay consolidar.py que ejecutar.", "Falló el paso anterior.", cont)
